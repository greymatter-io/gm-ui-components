...
describe("Status", () => {
  ...
  describe("Props", () => {
    ...
    test('should use the correct colors and graphic based on the status prop', () => {
      const testTable = [
        {statusType: "info", color: keen.COLOR_INTENT_INFO, visual: "GraphicStatusInfo"},
        {statusType: "progress", color: keen.COLOR_INTENT_SUCCESS, visual: "icon-mock"},
        {statusType: "warning", color: keen.COLOR_INTENT_WARNING, visual: "GraphicStatusWarning"},
        {statusType: "error", color: keen.COLOR_INTENT_DANGER, visual: "GraphicStatusError"},
        {statusType: "success", color: keen.COLOR_INTENT_SUCCESS, visual: "GraphicStatusSuccess"},
        {statusType: "indeterminate", color: keen.COLOR_INTENT_SUCCESS, visual: "GraphicStatusIndeterminate"},
      ]
      testTable.forEach(t => {
        aStatus = mount(<Status statusType={t.statusType} />);
        expect(aStatus.find(t.visual)).toHaveLength(1);
        expect(aStatus.find("Graphic").props().graphicColor({theme: keen})).toBe(t.color);
      })
    });
  })
